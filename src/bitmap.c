...
int BYTES_PER_PIXEL; /// red, green, & blue
...
{
    bitmap_init();
    int actualSize = size * k;
    unsigned char image[actualSize][actualSize][BYTES_PER_PIXEL];

    int tbi = 0, tbj = 0, counterI = 0, counterJ = 0;
    ...
    generateBitmapImage(image, actualSize, actualSize, fileName);
}